#!/bin/bash
#SBATCH --job-name=run3_sentiment
#SBATCH --partition=gpu
#SBATCH --gres=gpu:1
#SBATCH --cpus-per-task=4
#SBATCH --mem=32G
#SBATCH --time=04:00:00
#SBATCH --output=logs/run3_sentiment_%j.out
#SBATCH --error=logs/run3_sentiment_%j.err
...
OUTBASE="/work/FAC/FDCA/IDHEAP/mhinterl/parp/SWISSDOX_REPO/data/processed/swissdox_sentences_with_s_t_s"
...
python scripts/run3_pipeline.py \
  --input  "/work/FAC/FDCA/IDHEAP/mhinterl/parp/SWISSDOX_REPO/data/processed/swissdox/swissdox_sentences_with_s_t.parquet" \
  --output_base "$OUTBASE" \
