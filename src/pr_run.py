...
Result = Tuple[str, int, int, str]  # (t, sp, p, j)
...
def _load_ckpt(p: Path) -> Dict[str, Result]:
    f = _ckpt(p)
    if not f.exists():
        return {}
    ...
    has_j = "j" in d.columns

    out: Dict[str, Result] = {}
    for i in range(len(d)):
        sid = str(d.loc[i, "sentence_id"])
        t = str(d.loc[i, "t"])
        sp = int(d.loc[i, "sp"])
        pp = int(d.loc[i, "p"])
        j = str(d.loc[i, "j"]) if has_j else ""
...
def _save_ckpt(p: Path, m: Dict[str, Result]) -> None:
...
def _parse(txt: str) -> Dict[str, Result]:
...
def run(cfg: Cfg) -> None:
    ...
    for gi in range(0, len(prompts), cfg.prompts_per_batch):
        ...
        for att in range(3):
            try:
                ...
                for j_idx, (items, up, raw) in enumerate(zip(grp, ups, outs)):
                    got = _parse(raw)
                    for sid, _ in items:
                ...
                try:
                    ...
                    _write_failure(
                        ...
                        tag,
                        ups[0] if ups else "",
                        outs[0] if "outs" in locals() and outs else f"EXC: {e}",
                    )
                except Exception:
                    pass
                time.sleep(2 * (att + 1))
    ...
    df["j"] = df["sentence_id"].map(lambda x: res.get(str(x), ("Other", 0, 0, ""))[3])

    out_parq = cfg.outdir / "swissdox_sentences_with_t_sp_p_j.parquet"
    out_csv = cfg.outdir / "swissdox_sentences_with_t_sp_p_j.csv"
    df.to_parquet(out_parq, index=False)
    df.to_csv(out_csv, index=False)

    _save_ckpt(cfg.scratch, dict(zip(
        df["sentence_id"].astype(str),
        zip(df["t"].astype(str), df["sp"].astype(int), df["p"].astype(int), df["j"].astype(str))
    )))
